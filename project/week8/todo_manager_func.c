...
void addTask() { //할 일 추가하는 함수
	...
	scanf_s("%s", tasks[taskCount],(int)sizeof(tasks[taskCount])); //사용자에게 할 일 입력 후 tasks배열에 저장
...
void changeTask() {
	...
	scanf_s("%s", tasks[changeTask - 1],(int)sizeof(tasks[changeTask -1])); //수정 할 일의 번호를 입력받고 리스트를 수정
...
int main() {
	...
	while (1) {
		int choice; //사용자의 입력값을 받는 변수 생성
		int terminate = 0; //프로그램을 종료하는 변수 생성
